...
class ModelArguments:
    ...
    our_modified: bool = field(
        default=False,
        metadata={
            "help": "whether use our modified model"
        }
    )
    auto_pos: bool = field(
        default=False,
        metadata={
            "help": "Whether to use the idential hard positive example like unsup-simcse. "
        },
    )
    in_batch_pos: bool = field(
        default=False,
        metadata={
            "help": "Whether to use in batch positive example. "
        },
    )
    weighted: bool = field(
        default=False,
        metadata={
            "help": "if data has weight"
        }
    )
...
class DataTrainingArguments:
    ...
    max_choices: Optional[int] = field(
        default=5,
        metadata={
            "help": "The maximal number of choices"
        },
    )
...
def main():
    ...
    print(data_args.train_file, "len:", len(datasets["train"]))
    ...
    def prepare_features(examples):
        ...
        for key in sent_features:
            ...
            for idx in range(total):
                features[key].append([sent_features[key][idx]])

        features["labels"]=[]
        if model_args.weighted:
            for idx in range(total):
                sum=examples["label0"][idx]+examples["label1"][idx]
                features["labels"].append([examples["label0"][idx]/sum,examples["label1"][idx]/sum,1/examples["num_copies"][idx]])
        else:
            for idx in range(total):
                sum=examples["label0"][idx]+examples["label1"][idx]
